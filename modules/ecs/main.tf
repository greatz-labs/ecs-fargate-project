  ...
  container_definitions = jsonencode([
    {
      ...
      # Add secrets here when needed:
      # secrets = [{ name = "DB_PASSWORD", valueFrom = "arn:aws:ssm:region:account:parameter/..." }]
      ...
      # Container-level health check — requires curl in the image.
      # Remove this block if your base image doesn't include curl.
      healthCheck = {
...
# ── Task security group ───────────────────────────────────────────────────────
# Inbound restricted to ALB SG only; egress open for ECR, CloudWatch, SSM.
...
resource "aws_security_group" "task" {
  ...
  egress {
    description = "Outbound for ECR pulls, CloudWatch, SSM"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_ecs_service" "this" {
  ...
  # Rolling deploy: keep 100% capacity, bring up new tasks before draining old ones
