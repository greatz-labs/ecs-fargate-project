...
# Grants ECS agent access to pull secrets at task start time
    ...
    Statement = [
      {
        ...
        # Covers both /project/env/* and project/env/* naming conventions
        Resource = [
      ...
      },
      {
        ...
        Effect   = "Allow"
        Action   = ["kms:Decrypt"]
        Resource = "*" # tighten to specific KMS key ARN if using a CMK
      }
    ]
  })
}
...
# ── Task role ──────────────────────────────────────────────────────────────────
# Used by application code for AWS API calls (S3, DynamoDB, etc.).
# Extend via task_role_policy_arns.
